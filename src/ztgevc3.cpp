 ...
 * =====================================================================
 */
inline int izlapb(int curr, int bsize)
{
    int result;
 ...
 * =====================================================================
 */
inline int izlanb(int n, int curr, int bsize)
{
    int result;
...
void zlalsr(int ldS, const std::complex<double> *S, int ldP, const std::complex<double> *P, int m_size, int ldV, std::complex<double> *rhs_loc, std::complex<double> *X_panel_base, int panel_rows, int nb, const std::complex<double> *alpha, const std::complex<double> *beta, int is_diag, std::complex<double> *work, double ascale, double bscale, double safemin, double bignum, const int *col_map, int nb_sel)
{
    ...
    double t, rhs_max, scale;
    std::complex<double> *work_rhs;
    ...
    for (k = 0; k < nb; k++) {
        ...
        t = 1.0 / std::max({ std::abs(a) * ascale, std::abs(b_val) * bscale, safemin });
        acoeff = std::complex<double>(t * ascale, 0.0) * b_val * std::complex<double>(bscale, 0.0);
        bcoeff = std::complex<double>(t * bscale, 0.0) * a * std::complex<double>(ascale, 0.0);
        ...
        if (is_diag) {
            rhs_loc[k + c_packed * ldV] = { 1.0, 0.0 };
        }
 ...
 * Purpose:
 * =======
 * ZTGEVC3 computes some or all of the right and/or left eigenvectors of
 ...
 *
 * VL              (input/output) std::complex<double>*
 * Left eigenvector storage.
 *
 * ldvl            (input) int
 * Leading dimension of array VL.
 *
 * VR              (input/output) std::complex<double>*
...
void ztgevc3(char side, char howmny, const int *select, int n, const std::complex<double> *S, int lds, const std::complex<double> *P, int ldp, const std::complex<double> *alpha, const std::complex<double> *beta, std::complex<double> *VL, int ldvl, std::complex<double> *VR, int ldvr, int mm, int *m, std::complex<double> *work, int lwork, int *info)
{
    ...
    std::complex<double> *X_panel, *Temp, *work_local;
    int col_map[128];
    std::complex<double> alpha_m1, beta_1, alpha_1, one, zero;
    int current_out_col, curr_col, i, nb, ld_x, nb_sel, c, r;
    int curr_row, j, j_nb, is_diag, out_col, c_packed;
    std::complex<double> *TempS, *TempP;
    std::complex<double> a, b_val, acoeff, bcoeff, xr;
    double t, update_max, safe_limit, scale;
    ...
    if (lwork == -1) {
        work[0] = std::complex<double>(static_cast<double>(req_lwork), 0.0);
        *info = 0;
        return;
    }
    ...
    *info = 0;
    ...
    alpha_m1 = std::complex<double>(-1.0, 0.0);
    beta_1 = std::complex<double>(1.0, 0.0);
    alpha_1 = std::complex<double>(1.0, 0.0);
    one = std::complex<double>(1.0, 0.0);
    ...
    if (compute_right && VR != nullptr) {
        ...
        while (curr_col > 0) {
            ...
            while (curr_row > 0) {
                ...
                zlalsr(lds, &S[j + j * lds], ldp, &P[j + j * ldp], j_nb, ld_x, &X_panel[j], X_panel, ld_x, nb, alpha + i, beta + i, is_diag, work_local, ascale, bscale, safemin, bignum, col_map, nb_sel);
                ...
                if (j > 0) {
                    ...
                    // Perform block update accumulating results into X_panel via ZGEMM
                    zgemm_("N", "N", &j, &nb_sel, &j_nb, &alpha_m1, &S[0 + j * lds], &lds, TempS, &j_nb, &beta_1, &X_panel[0], &ld_x);
                    zgemm_("N", "N", &j, &nb_sel, &j_nb, &alpha_1, &P[0 + j * ldp], &ldp, TempP, &j_nb, &beta_1, &X_panel[0], &ld_x);
                }
            ...
            if (do_back) {
                zgemm_("N", "N", &n, &nb_sel, &curr_col, &one, VR, &ldvr, X_panel, &ld_x, &zero, Temp, &n);
                for (c = 0; c < nb_sel; c++) {
    ...
    if (compute_left && VL != nullptr) {
        ...
        while (curr_col < n) {
            ...
            i_next = izlanb(n, curr_col, bsize);
            nb = i_next - curr_col;
            i = curr_col;
            rem_rows = n - i;
            ld_x = rem_rows;
            ...
            while (curr_row < n) {
                ...
                j_next = izlanb(n, curr_row, bsize);
                j_nb = j_next - curr_row;
                is_diag = (curr_row == i) ? 1 : 0;

                // Extract views and perform Local solve
                zlalsl(lds, &S[curr_row + curr_row * lds], ldp, &P[curr_row + curr_row * ldp], j_nb, ld_x, &X_panel[curr_row - i], X_panel, ld_x, nb, alpha + i, beta + i, is_diag, work_local, ascale, bscale, safemin, bignum, col_map, nb_sel);
                ...
                if (j_next < n) {
                    ...
                    zgemm_("C", "N", &rem, &nb_sel, &j_nb, &alpha_m1, &S[curr_row + j_next * lds], &lds, TempS, &j_nb, &beta_1, &X_panel[j_next - i], &ld_x);
                    zgemm_("C", "N", &rem, &nb_sel, &j_nb, &alpha_1, &P[curr_row + j_next * ldp], &ldp, TempP, &j_nb, &beta_1, &X_panel[j_next - i], &ld_x);
                }
            ...
            // via zgemm if computing original coordinates (howmny = 'B')
            if (do_back) {
                zgemm_("N", "N", &n, &nb_sel, &rem_rows, &one, &VL[i * ldvl], &ldvl, X_panel, &ld_x, &zero, Temp, &n);
                for (c = 0; c < nb_sel; c++) {
                    out_col = current_out_col + c;
                    for (r = 0; r < n; r++) {
