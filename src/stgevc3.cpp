...
{
    int j, k, pivot_row;
    float mult, alpha, eps, safemin, local_max, perturb;
    ...
    for (j = 0; j < n - 1; j++) {
        ...
        if (std::abs(A[j + j * lda]) < perturb) {
            A[j + j * lda] = (A[j + j * lda] < 0.0f) ? -perturb : perturb;
        }
    ...
    if (std::abs(A[(n - 1) + (n - 1) * lda]) < perturb) {
        A[(n - 1) + (n - 1) * lda] = (A[(n - 1) + (n - 1) * lda] < 0.0f) ? -perturb : perturb;
    }
...
int slau2s(int n, int nrhs, float *A, int lda, float *B, int ldb)
{
    ...
    char side = 'L', uplo = 'U', transa = 'N', diag = 'N';
    float eps, safemin, local_max, perturb;
...
int slalhs(int n, int nrhs, float *A, int lda, int *jpiv, float *B, int ldb)
{
    ...
    for (k = 0; k < n - 1; k++) {
        ...
        if (std::abs(pivot) < perturb) {
            pivot = (pivot < 0.0f) ? -perturb : perturb;
            A[k + k * lda] = pivot;
        }
 ...
 * Purpose:
 * =======
 * STGEVC3 computes some or all of the right and/or left eigenvectors of
...
void stgevc3(char side, char howmny, const int *select, int n, const float *S, int lds, const float *P, int ldp, const float *alphar, const float *alphai, const float *beta, float *VL, int ldvl, float *VR, int ldvr, int mm, int *m, float *work, int lwork, int *info)
{
    ...
    float ascale, bscale;
    float t, acoeff, bcoeffR, bcoeffI;
    float update_max, safe_limit, scale, xmax;
        ...
        while (curr_col > 0) {
            ...
            while (curr_row > 0) {
                ...
                if (j > 0) {
                    ...
                    // Perform block update accumulating results into X_panel via SGEMM
                    alpha_m1 = -1.0f;
                    beta_1 = 1.0f;
                    alpha_1 = 1.0f;
