      SUBROUTINE STGEVC3(SIDE, HOWMNY, SELECT, N, S, LDS, P, LDP,
     ...
     $                   MM, M, WORK, LWORK, INFO)
      ...
      INTEGER ISLAPB, ISLANB
      EXTERNAL ISLAPB, ISLANB
      EXTERNAL SLAUHS, SLAU2S, SLALHS, SLAL2S, SLALSR, SLALSL
      EXTERNAL SGEMM
...
* Pass 1: Count total selected eigenvalues
      ...
      DO WHILE (K_IDX .LE. N)
         ...
         IF (DO_SEL) THEN
            ...
            IF (ALPHAI(K_IDX) .NE. 0.0E0 .AND. K_IDX .LT. N) THEN
...
* ==================================================================
     ...
     $                I + C - 1 .LT. N) THEN
            ...
            DO C = 1, NB_SEL
               DO R = 1, CURR_COL
                  WORK(X_PANEL_OFFSET + R + (C - 1) * LD_X) = 0.0E0
               END DO
            END DO
            ...
            DO WHILE (CURR_ROW .GT. 0)
               ...
               CALL SLALSR(LDS, S(J, J), LDP, P(J, J), J_NB, LD_X,
     ...
     $                     SAFEMIN, BIGNUM, COL_MAP, NB_SEL)
               ...
               IF (J .GT. 1) THEN
                  ...
                  DO WHILE (C .LE. NB)
                     ...
                     T = 1.0E0 / MAX(ABS(AR) * ASCALE + ABS(AI) *
     ...
     $                  IDX)))
                  ...
                  SAFE_LIMIT = BIGNUM / REAL(MAX(1, NB_SEL))
     ...
     $                          (C_IDX - 1) * LD_X) * SCALE
                  ...
                  BETA_1 = 1.0E0
                  ALPHA_1 = 1.0E0
                  CALL SGEMM('N', 'N', J - 1, NB_SEL, J_NB, ALPHA_M1,
     $                       S(1, J), LDS, 
     $                       WORK(WORK_LOCAL_OFFSET + 1), J_NB, BETA_1,
     $                       WORK(X_PANEL_OFFSET + 1), LD_X)
                  CALL SGEMM('N', 'N', J - 1, NB_SEL, J_NB, ALPHA_1,
     $                       P(1, J), LDP, 
     ...
     $                       WORK(X_PANEL_OFFSET + 1), LD_X)
            ...
            IF (DO_BACK) THEN
               ONE = 1.0E0
               ZERO = 0.0E0
               CALL SGEMM('N', 'N', N, NB_SEL, CURR_COL, ONE, VR, LDVR,
...
* ==================================================================
     ...
     $                I + C - 1 .LT. N) THEN
            ...
            DO WHILE (CURR_ROW .LE. N)
               ...
               CALL SLALSL(LDS, S(CURR_ROW, CURR_ROW), LDP, 
     ...
     $                          (C_IDX - 1) * LD_X) * SCALE
                  ...
                  BETA_1 = 1.0E0
                  ALPHA_1 = 1.0E0
                  CALL SGEMM('T', 'N', REM, NB_SEL, J_NB, ALPHA_M1,
     $                       S(CURR_ROW, J_NEXT + 1), LDS, 
     $                       WORK(WORK_LOCAL_OFFSET + 1), J_NB, BETA_1,
     $                       WORK(X_PANEL_OFFSET + J_NEXT - I + 2), 
     $                       LD_X)
                  CALL SGEMM('T', 'N', REM, NB_SEL, J_NB, ALPHA_1,
     ...
     $                       LD_X)
            ...
            IF (DO_BACK) THEN
               ONE = 1.0E0
               ZERO = 0.0E0
               CALL SGEMM('N', 'N', N, NB_SEL, REM_ROWS, ONE, 
