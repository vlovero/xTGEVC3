     ...
     $                   LWORK, INFO)
      ...
      LOGICAL SELECT(*)
      COMPLEX*16 S(LDS, *), P(LDP, *), VL(LDVL, *), VR(LDVR, *)
      COMPLEX*16 ALPHA(*), BETA(*), WORK(*)
      ...
      INTEGER COL_MAP(128)
      COMPLEX*16 ALPHA_M1, BETA_1, ALPHA_1, ONE, ZERO
...
*     External functions
      ...
      EXTERNAL DLAMCH, LSAME, IZLAPB, IZLANB, ZGEMM, ZLALSR, ZLALSL
     ...
     $                    2 * (BSIZE + 1)
      ...
      IF (LWORK .EQ. -1) THEN
          WORK(1) = DCMPLX(DBLE(REQ_LWORK), 0.0D0)
          INFO = 0
          RETURN
      END IF
      ...
      ALPHA_M1 = DCMPLX(-1.0D0, 0.0D0)
      BETA_1 = DCMPLX(1.0D0, 0.0D0)
      ALPHA_1 = DCMPLX(1.0D0, 0.0D0)
      ...
      IF (COMP_R .AND. LDVR .GT. 0) THEN
          ...
          DO WHILE (CURR_COL .GT. 0)
              ...
              DO WHILE (CURR_ROW .GT. 0)
                  ...
                  CALL ZLALSR(LDS, S(J, J), LDP, P(J, J), J_NB, LD_X,
     $                        WORK(XPANEL_PTR + J - 1),
                  ...
                  IF (J .GT. 1) THEN
                      ...
                      DO C = 1, NB
                          ...
                          IF (C_PACKED .GT. 0) THEN
                              A = ALPHA(I + C - 1)
                              B_VAL = BETA(I + C - 1)
                              T = 1.0D0 / MAX(ABS(A) * ASCALE,
     $                                        ABS(B_VAL) * BSCALE,
     $                                        SAFEMIN)
                              ACOEFF = DCMPLX(T * ASCALE, 0.0D0) *
     $                                 B_VAL * DCMPLX(BSCALE, 0.0D0)
                              BCOEFF = DCMPLX(T * BSCALE, 0.0D0) *
     $                                 A * DCMPLX(ASCALE, 0.0D0)
     ...
     $                              BCOEFF * XR
                      ...
                      DO IDX = 1, TOT_ELEM
                          UPD_MAX = MAX(UPD_MAX,
     $                      ABS(DBLE(WORK(TS_PTR + IDX - 1))) +
     $                      ABS(DIMAG(WORK(TS_PTR + IDX - 1))),
     $                      ABS(DBLE(WORK(TP_PTR + IDX - 1))) +
     $                      ABS(DIMAG(WORK(TP_PTR + IDX - 1))))
                      END DO

                      SAFELIM = BIGNUM / DBLE(MAX(1, NB_SEL))
     ...
     $                              DCMPLX(SCALE_VAL, 0.0D0)
                      ...
                      CALL ZGEMM('N', 'N', J - 1, NB_SEL, J_NB,
     $                           ALPHA_M1, S(1, J), LDS,
     ...
     $                           WORK(XPANEL_PTR), LD_X)
              ...
              IF (DO_BACK) THEN
                  CALL ZGEMM('N', 'N', N, NB_SEL, CURR_COL, ONE,
      ...
      IF (COMP_L .AND. LDVL .GT. 0) THEN
          ...
          DO WHILE (CURR_COL .LE. N)
              ...
              DO WHILE (CURR_ROW .LE. N)
                  ...
                  CALL ZLALSL(LDS, S(CURR_ROW, CURR_ROW), LDP,
     ...
     $                              DCMPLX(SCALE_VAL, 0.0D0)
                      ...
                      CALL ZGEMM('C', 'N', REM, NB_SEL, J_NB,
     ...
     $                           LD_X)
              ...
              IF (DO_BACK) THEN
                  CALL ZGEMM('N', 'N', N, NB_SEL, REM_ROWS, ONE,
