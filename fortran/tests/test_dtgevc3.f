      ...
      DOUBLE PRECISION, ALLOCATABLE :: WORK(:), WORK_LAPACK(:)
      DOUBLE PRECISION S(N,N), P(N,N), ALPHAR(N), ALPHAI(N), BETA(N)
      DOUBLE PRECISION VR(N,N), VL(N,N), RES3(2), RESL(2)
      LOGICAL SEL(1)

      PRINT *, '--- Testing 4x4 Static Matrix (Both Left/Right) ---'

      S(1,1) = 1.0D0; S(1,2) = 2.0D0; S(1,3) = 3.0D0; S(1,4) = 4.0D0
      S(2,1) = 0.0D0; S(2,2) = 2.0D0; S(2,3) = 5.0D0; S(2,4) = -1.0D0
      S(3,1) = 0.0D0; S(3,2) = -5.0D0; S(3,3) = 2.0D0; S(3,4) = 2.0D0
      S(4,1) = 0.0D0; S(4,2) = 0.0D0; S(4,3) = 0.0D0; S(4,4) = 3.0D0

      P(1,1) = 2.0D0; P(1,2) = 1.0D0; P(1,3) = -1.0D0; P(1,4) = 3.0D0
      P(2,1) = 0.0D0; P(2,2) = 1.0D0; P(2,3) = 0.0D0; P(2,4) = 2.0D0
      P(3,1) = 0.0D0; P(3,2) = 0.0D0; P(3,3) = 1.0D0; P(3,4) = -1.0D0
      P(4,1) = 0.0D0; P(4,2) = 0.0D0; P(4,3) = 0.0D0; P(4,4) = 2.0D0
     ...
     $            2*(MAX_BSIZE+1)
      ...
      DO J = 1, N
         DO I = 1, N
            VR(I,J) = 0.0D0; VL(I,J) = 0.0D0
         END DO
         VR(J,J) = 1.0D0; VL(J,J) = 1.0D0
      END DO
      CALL DTGEVC3('B', 'B', SEL, N, S, N, P, N, ALPHAR, ALPHAI,
     $             BETA, VL, N, VR, N, N, M_OUT, WORK, WORK_SIZE, INFO)
      CALL DGET52(.TRUE., .TRUE., N, S, N, P, N, ALPHAR, ALPHAI,
     $            BETA, VL, N, VR, N, RES3)
      ...
      DO J = 1, N
         ...
         VR(J,J) = 1.0D0; VL(J,J) = 1.0D0
      END DO
      CALL DTGEVC('B', 'B', SEL, N, S, N, P, N, VL, N, VR, N, N,
     $            M_OUT, WORK_LAPACK, INFO)
      CALL DGET52(.TRUE., .TRUE., N, S, N, P, N, ALPHAR, ALPHAI,
     $            BETA, VL, N, VR, N, RESL)

      WRITE(*, '(A, 1PD14.6)') 
     $   '[dtgevc3] dget52 Max Right Error: ', RES3(1)
      WRITE(*, '(A, 1PD14.6)') 
     $   '[dtgevc3] dget52 Max Left Error:  ', RES3(2)
      WRITE(*, '(A, 1PD14.6)') 
     $   '[lapack ] dget52 Max Right Error: ', RESL(1)
      WRITE(*, '(A, 1PD14.6)') 
     $   '[lapack ] dget52 Max Left Error:  ', RESL(2)
      PRINT *
      ...
      P(2,1) = 0.0D0; P(2,2) = 1.0D0; P(2,3) = 0.0D0; P(2,4) = 2.0D0
      P(3,1) = 0.0D0; P(3,2) = 0.0D0; P(3,3) = 1.0D0; P(3,4) = -1.0D0
      P(4,1) = 0.0D0; P(4,2) = 0.0D0; P(4,3) = 0.0D0; P(4,4) = 0.0D0
     ...
     $      '[lapack ] dget52 Max Left Error:  ', RESL(2)
      ...
      DOUBLE PRECISION, ALLOCATABLE :: WORK(:), WORK_LAPACK(:)
      DOUBLE PRECISION DUMMY(1), RES3(2), RESL(2)
      REAL T1, T2
     ...
     $             BETA, VL, N, VR, N, N, M_OUT, WORK, LWORK, INFO)
      CALL CPU_TIME(T2)
      PRINT *, 'dtgevc3 (''B'') took ', T2-T1, ' seconds'
      CALL DGET52(.TRUE., .TRUE., N, S, N, P, N, ALPHAR, ALPHAI,
     $            BETA, VL, N, VR, N, RES3)
     ...
     $            M_OUT, WORK_LAPACK, INFO)
      CALL CPU_TIME(T2)
      PRINT *, 'LAPACK dtgevc (''B'') took ', T2-T1, ' seconds'
      CALL DGET52(.TRUE., .TRUE., N, S, N, P, N, ALPHAR, ALPHAI,
     $            BETA, VL, N, VR, N, RESL)
     ...
     $      '  [lapack ] Max R = ', RESL(1), ' L = ', RESL(2)
      ...
      SUBROUTINE DGET52(COMP_L, COMP_R, N, S, LDS, P, LDP,
     $                  ALPHAR, ALPHAI, BETA, VL, LDVL, VR, LDVR, RES)
      IMPLICIT NONE
      LOGICAL COMP_L, COMP_R
      INTEGER N, LDS, LDP, LDVL, LDVR
      DOUBLE PRECISION S(LDS,*), P(LDP,*), ALPHAR(*), ALPHAI(*), BETA(*)
      DOUBLE PRECISION VL(LDVL,*), VR(LDVR,*), RES(2)
      DOUBLE PRECISION VAL, RR_VAL, RI_VAL, V_R, V_I
      DOUBLE PRECISION S_VAL, P_VAL, DEN, A_NORM
      DOUBLE PRECISION NORMS, NORMP, CSS, CSP, A, B, AR, AI
     ...
     $   LDP, ALPHAR, ALPHAI, BETA)
      ...
      DO C = 1, N
         DO R = 1, C
            ...
            S(R,C) = RAND_VAL * 2.0D0 - 1.0D0
            CALL RANDOM_NUMBER(RAND_VAL)
            P(R,C) = RAND_VAL * 2.0D0 - 1.0D0
         END DO
         IF (P(C,C) .GE. 0.0D0) THEN
            P(C,C) = P(C,C) + 1.0D0
         ELSE
            P(C,C) = P(C,C) - 1.0D0
         END IF
     ...
     $                                       ALPHAR, ALPHAI, BETA)
      ...
      INTEGER I, J, K
      DOUBLE PRECISION BIG, SMALL, A, B, P_VAL, RAND_VAL, RAND_VAL2
      ...
      CASE (8)
         DO I = 1, N
            S(I,I) = DBLE(I) / DBLE(N)
            P(I,I) = DBLE(N - I + 1) / DBLE(N)
         END DO
      CASE (9)
         DO I = 1, N
            S(I,I) = BIG * DBLE(I) / DBLE(N)
            P(I,I) = SMALL
         END DO
      CASE (10)
         DO I = 1, N
            S(I,I) = SMALL * DBLE(I) / DBLE(N)
            P(I,I) = BIG
         END DO
      CASE (11)
         DO I = 1, N
            S(I,I) = BIG
            P(I,I) = SMALL * DBLE(I) / DBLE(N)
         END DO
      CASE (12)
         DO I = 1, N
            S(I,I) = SMALL
            P(I,I) = BIG * DBLE(I) / DBLE(N)
         END DO
      CASE (13)
         DO I = 1, N
            S(I,I) = BIG * DBLE(I) / DBLE(N)
         ...
         IF (MOD(TYPE, 2) .EQ. 0) THEN
            ...
            DO WHILE (I .LT. N)
               ...
               IF (RAND_VAL .GT. 0.5D0) THEN
                  ...
                  P_VAL = ABS(RAND_VAL2 * 2.0D0 - 1.0D0) + 0.1D0
