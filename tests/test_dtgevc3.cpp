...
// Replicates the LAPACK test routine dget52.
// Evaluates the scaled backward error for left and right generalized eigenvectors.
// result[0] = max right error ratio
// result[1] = max left error ratio
void dget52(bool compute_left, bool compute_right, int n, const double *S, int lds, const double *P, int ldp, const double *alphar, const double *alphai, const double *beta, const double *VL, int ldvl, const double *VR, int ldvr, double *result)
{
    double normS, normP, csS, csP, a, b, aR, aI, norm_r, norm_v;
    double val, rr_val, ri_val, v_r, v_i, s_val, p_val, den, a_norm;
...
void generate_lapack_matrix_type(int type, int n, double *S, int lds, double *P, int ldp, double *alphar, double *alphai, double *beta)
{
    int i, j, k;
    double big, small, a, b, p;
    ...
    case 7:
        for (i = 0; i < n; i++) {
            S[i + i * lds] = 1.0;
            P[i + i * ldp] = (double)(i + 1) / n;
        }
        break;
    case 8:
        for (i = 0; i < n; i++) {
            S[i + i * lds] = (double)(i + 1) / n;
            P[i + i * ldp] = (double)(n - i) / n;
        }
        break;
    case 9:
        for (i = 0; i < n; i++) {
...
void test_dtgevc()
{
    ...
    double *work, *work_lapack, res3[2], resL[2];
    ...
    dget52(true, true, n, S, n, P, n, alphar, alphai, beta, VL, n, VR, n, res3);
    ...
    dtgevc_("B", "B", nullptr, &n, S, &n, P, &n, VL, &n, VR, &n, &n, &m_out, work_lapack, info);
    dget52(true, true, n, S, n, P, n, alphar, alphai, beta, VL, n, VR, n, resL);

    fmt::print("[dtgevc3] dget52 Max Right Error Ratio: {:e}\n", res3[0]);
    fmt::print("[dtgevc3] dget52 Max Left Error Ratio:  {:e}\n", res3[1]);
    fmt::print("[lapack ] dget52 Max Right Error Ratio: {:e}\n", resL[0]);
    fmt::print("[lapack ] dget52 Max Left Error Ratio:  {:e}\n\n", resL[1]);
...
void test_dtgevc_scaling()
{
    ...
    double s, *work, *work_lapack, res3[2], resL[2];
...
{
    double *S, *P, *VR, *VL, *alphar, *alphai, *beta, *work, *work_lapack, dummy[1], res3[2], resL[2];
    ...
    time_code("dtgevc3 ('B')", [&]() { dtgevc3('B', 'B', nullptr, n, S, n, P, n, alphar, alphai, beta, VL, n, VR, n, n, &m_out, work, lwork, info); });
    dget52(true, true, n, S, n, P, n, alphar, alphai, beta, VL, n, VR, n, res3);
    ...
    time_code("LAPACK dtgevc ('B')", [&]() { dtgevc_("B", "B", nullptr, &n, S, &n, P, &n, VL, &n, VR, &n, &n, &m_out, work_lapack, info); });
    dget52(true, true, n, S, n, P, n, alphar, alphai, beta, VL, n, VR, n, resL);
...
void test_lapack_types(int n)
{
    ...
    for (type = 1; type <= 26; type++) {
        ...
        dtgevc3('B', 'B', nullptr, n, S, n, P, n, alphar, alphai, beta, VL, n, VR, n, n, &m_out, work, lwork, info);
        dget52(true, true, n, S, n, P, n, alphar, alphai, beta, VL, n, VR, n, res3);
        ...
        fmt::println("  [dtgevc3] Max Right = {:e}, Max Left = {:e}", res3[0], res3[1]);
